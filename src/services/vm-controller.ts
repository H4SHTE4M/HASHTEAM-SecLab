...
export const DEFAULT_VM_ASSETS: VmAssetConfig = {
  // 开发时直接使用 public/；生产构建注入内容寻址目录。VM 整组资源共享
  // 同一个目录 hash，确保 libv86、wasm、内核和 rootfs 不会跨版本混用。
  libv86Url: `${__VM_ASSET_BASE__}v86/libv86.js`,
  wasmUrl: `${__VM_ASSET_BASE__}v86/v86.wasm`,
  biosUrl: `${__VM_ASSET_BASE__}v86/bios/seabios-256k.bin`,
  bzimageUrl: `${__VM_ASSET_BASE__}vm/bzImage`,
  initrdUrl: `${__VM_ASSET_BASE__}vm/rootfs.cpio.gz`,
...
function loadLibV86(url: string): Promise<void> {
  ...
  scriptLoadingPromise = new Promise<void>((resolve, reject) => {
    ...
    scriptLoadingElement = script
    rejectScriptLoading = reject
    script.src = url
    script.async = true
    script.onload = () => {
      scriptLoadingElement = null
      rejectScriptLoading = null
...
/** 启动超时/页面卸载时解除悬挂的 script 请求，保证“重试”会发起新请求。 */
function cancelLibV86Load(): void {
  if (scriptLoadingElement === null || window.V86 !== undefined) return
  const script = scriptLoadingElement
  const reject = rejectScriptLoading
  script.onload = null
  script.onerror = null
  script.remove()
  scriptLoadingPromise = null
  scriptLoadingElement = null
  rejectScriptLoading = null
  reject?.(new Error('虚拟机启动已取消'))
}

/** 预检静态资源可达性，给出明确的缺失错误（而非 v86 内部的模糊失败） */
async function assertAssetsReachable(urls: string[], signal: AbortSignal): Promise<void> {
  await Promise.all(
    urls.map(async (url) => {
...
export class V86Controller implements VirtualMachineController {
  ...
  private readonly byteHandler = (byte: number): void => {
    ...
    if (!this.serialLogLineTruncated) {
      if (this.serialLogLine.length + text.length <= MAX_SERIAL_LOG_LINE_LENGTH) {
        this.serialLogLine += text
      } else {
        const remaining = MAX_SERIAL_LOG_LINE_LENGTH - this.serialLogLine.length
        if (remaining > 0) this.serialLogLine += text.slice(0, remaining)
        this.serialLogLineTruncated = true
        if (text.includes('\n')) this.serialLogLine += '\n'
      }
    } else if (text.includes('\n')) {
      // 截断状态下只关心下一次换行，不继续累积攻击者可控的无界内容。
      this.serialLogLine += '\n'
    }
    let nl: number
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      await assertAssetsReachable(assetUrls, startAbortController.signal)
      this.assertStartActive()
      ...
      await this.assertWasmCompilable(startAbortController.signal)
      this.assertStartActive()
   ...
   */
  private async assertWasmCompilable(signal: AbortSignal): Promise<void> {
    const fallbackUrl = this.assets.wasmUrl.replace('v86.wasm', 'v86-fallback.wasm')
    if (await this.canCompileWasm(this.assets.wasmUrl, signal)) {
      log('boot', 'wasm 主变体可编译')
      return
    }
    this.assertStartActive()
    log('boot', 'wasm 主变体编译失败，检查 fallback 变体', 'warn')
    if (await this.canCompileWasm(fallbackUrl, signal)) {
  ...
  private async canCompileWasm(url: string, signal: AbortSignal): Promise<boolean> {
    try {
