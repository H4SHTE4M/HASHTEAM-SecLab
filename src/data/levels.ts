    ...
    guide: [
      {
        ...
        note:
          '先认识管道 |：它把左边命令的输出，当作右边命令的输入。' +
          '第一步只用 grep，把所有含「Failed password」的失败登录行筛出来，看看长什么样。',
      },
      {
        command: 'grep "Failed password" auth.log | head -n 1',
        note:
          '不确定 IP 在第几列？在管道中段接一个 head -n 1 只看第一行，' +
          '按空格数一数 IP 是第几个字段（这关里是第 11 列）。',
      },
      {
        command: 'grep "Failed password" auth.log | awk \'{print $11}\'',
        note:
          'awk 按空格切分每一行，{print $11} 打印第 11 列——也就是来源 IP。' +
          '现在输出里每行都是一个 IP 地址了。',
      },
      {
        command: 'grep "Failed password" auth.log | awk \'{print $11}\' | sort',
        note:
          'sort 给 IP 排序，相同的 IP 会排到一起。这一步是为下一步做准备：' +
          'uniq 只会合并「相邻」的重复行，所以必须先 sort。',
      },
      {
        command: 'grep "Failed password" auth.log | awk \'{print $11}\' | sort | uniq -c',
        note:
          'uniq -c 把相邻的重复 IP 合并，并在行首标出出现次数。' +
          '现在每个 IP 出现几次一目了然，只是还没排序。',
      },
      {
        command:
          'grep "Failed password" auth.log | awk \'{print $11}\' | sort | uniq -c | sort -nr | head',
        note:
          '最后再接 sort -nr 按次数从大到小排，head 取最前面的几行。' +
          '排在第一的那个 IP 就是失败次数最多的攻击者，交给 check 完成验证。',
      },
    ],
    hints: [
