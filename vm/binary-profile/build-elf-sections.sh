...
LAB="$ROOT/vm/binary-profile/elf-sections-01"
OUT="$ROOT/vm/rootfs-overlay/opt/pwnhub/labs/elf-sections-01/elf-sections"
[ "$#" -eq 0 ] || OUT="$1"
...
[ "$(LC_ALL=C "$CC" --version | sed -n '1p')" = "$EXPECTED_VERSION" ] || {
    echo 'compiler version does not match elf-sections-01/toolchain.lock' >&2
    exit 1
}
...
for section in .text .rodata .data .bss; do
    LC_ALL=C readelf -SW "$OUT" | grep -F " $section " >/dev/null || {
        echo "$section section missing" >&2
        exit 1
    }
done
for symbol in _start prepare_sections section_message initialized_counter pending_counter; do
...
sha256sum "$OUT"
LC_ALL=C readelf -h "$OUT" | grep -E 'Entry point address|Start of section headers|Number of section headers'
LC_ALL=C readelf -SW "$OUT" | grep -E ' \.(text|rodata|data|bss) '
"$OUT"
