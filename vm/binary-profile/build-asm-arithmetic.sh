...
[ "$(LC_ALL=C "$CC" --version | sed -n '1p')" = "$EXPECTED_VERSION" ] || {
    echo "compiler version does not match asm-arithmetic-01/toolchain.lock" >&2
    exit 1
}
...
for mnemonic in add sub imul idiv and or xor; do
    LC_ALL=C objdump -d "$OUT" | grep -E "[[:space:]]${mnemonic}[[:space:]]" >/dev/null || {
        echo "$mnemonic instruction missing" >&2
        exit 1
    }
done
