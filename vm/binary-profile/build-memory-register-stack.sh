...
LAB="$ROOT/vm/binary-profile/memory-register-stack-01"
RUNTIME_LAB="$ROOT/vm/rootfs-overlay/opt/pwnhub/labs/memory-register-stack-01"
LOCK="$LAB/toolchain.lock"
OUT="${1:-$ROOT/vm/rootfs-overlay/opt/pwnhub/labs/memory-register-stack-01/memory-register-stack}"
...
[ "$(LC_ALL=C "$CC" --version | sed -n '1p')" = "$EXPECTED_VERSION" ] || {
    echo "compiler version does not match memory-register-stack-01/toolchain.lock" >&2
    exit 1
}
...
"$CC" \
    ...
    -o "$OUT" "$LAB/memory-register-stack.c"
...
for symbol in _start teaching_stack capture_stack_sequence; do
    LC_ALL=C nm -n "$OUT" | grep -E "[[:space:]][TtBbDd] ${symbol}$" >/dev/null || {
        echo "$symbol symbol missing" >&2
        exit 1
    }
done
LC_ALL=C objdump -d "$OUT" | grep -E '[[:space:]]push[[:space:]]+\$0x11111111' >/dev/null || {
    echo "first stack insertion is missing" >&2
    exit 1
}
LC_ALL=C objdump -d "$OUT" | grep -E '[[:space:]]push[[:space:]]+\$0x22222222' >/dev/null || {
    echo "second stack insertion is missing" >&2
    exit 1
}
[ "$(LC_ALL=C objdump -d "$OUT" | grep -Ec '[[:space:]]pop[[:space:]]+%eax')" -eq 2 ] || {
    echo "stack removal sequence is missing" >&2
    exit 1
}
