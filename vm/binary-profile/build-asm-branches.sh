...
LAB="$ROOT/vm/binary-profile/asm-branches-01"
RUNTIME_LAB="$ROOT/vm/rootfs-overlay/opt/pwnhub/labs/asm-branches-01"
LOCK="$LAB/toolchain.lock"
OUT="${1:-$ROOT/vm/rootfs-overlay/opt/pwnhub/labs/asm-branches-01/asm-branches}"
...
[ "$(LC_ALL=C "$CC" --version | sed -n '1p')" = "$EXPECTED_VERSION" ] || {
    echo "compiler version does not match asm-branches-01/toolchain.lock" >&2
    exit 1
}
...
LC_ALL=C objdump -d "$OUT" | grep -E '[[:space:]]je[[:space:]]' >/dev/null || {
    echo "je instruction missing" >&2
    exit 1
}
LC_ALL=C objdump -d "$OUT" | grep -E '[[:space:]]jg[[:space:]]' >/dev/null || {
    echo "jg instruction missing" >&2
    exit 1
}
LC_ALL=C objdump -d "$OUT" | grep -E '[[:space:]]test[[:space:]]' >/dev/null || {
    echo "test instruction missing" >&2
    exit 1
}
