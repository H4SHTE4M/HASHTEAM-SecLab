...
[ "$(LC_ALL=C "$CC" --version | sed -n '1p')" = "$EXPECTED_VERSION" ] || {
    echo "compiler version does not match asm-stack-ops-01/toolchain.lock" >&2
    exit 1
}
...
LC_ALL=C objdump -d "$OUT" | grep -E '[[:space:]]push[[:space:]]' >/dev/null || {
    echo "push instruction missing" >&2
    exit 1
}
LC_ALL=C objdump -d "$OUT" | grep -E '[[:space:]]pop[[:space:]]' >/dev/null || {
