...
LAB="$ROOT/vm/binary-profile/pwn-ret2win-01"
OUT="${1:-$ROOT/vm/rootfs-overlay/opt/pwnhub/labs/pwn-ret2win-01/ret2win}"
...
[ "$(LC_ALL=C "$CC" --version | sed -n '1p')" = "$EXPECTED_VERSION" ] || {
    echo "compiler version does not match vm/binary-profile/pwn-ret2win-01/toolchain.lock" >&2
    exit 1
}
...
LC_ALL=C nm -n "$OUT" | grep -qE '[[:space:]]T win$' || {
    echo "win symbol missing" >&2
    exit 1
}
sha256sum "$OUT"
LC_ALL=C nm -n "$OUT" | grep -E '[[:space:]]T (_start|win|vulnerable)$'
