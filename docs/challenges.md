...
- 二进制样本、工具链锁或实验脚本变更后，运行
  ...
  并重新构建 VM。
- `chapterId='number-bases'` 的两个实验（num-bases-01「三种写法，同一个数」、
  num-wrap-01「8 位计数器装满之后」）是最低门槛入口：不涉及内存、ELF 或汇编，
  只教同一字节的十进制/二进制/十六进制写法与固定位宽回绕。二者均为
  `kind: 'pwn'`、`environmentProfile: 'binary'`，样本二进制哈希已列表锁死，用
  `vm/binary-profile/build-pwn-lab.sh` 与各自 `toolchain.lock` 中锁定的 Ubuntu 24.04
  i686 交叉工具链可逐字节重建；目录为 `vm/labs/pwnhub/num-bases-01/`、`num-wrap-01/`。
- 原 vuln-first 六个实验拆为两章：`chapterId='vuln-logic'`（第一批漏洞·逻辑篇）
  含 vuln-weak-random-01、vuln-integer-overflow-01、vuln-race-condition-01——
  完全不涉及内存，引导里顺路引入源代码→编译→程序、变量与 if 判断；
  `chapterId='vuln-memory'`（第一批漏洞·内存篇）含 vuln-overwrite-variable-01、
  vuln-string-overflow-01、vuln-format-string-01。六者全部 `kind: 'pwn'`、
  `environmentProfile: 'binary'`，样本二进制哈希已列表锁死，用
  `vm/binary-profile/build-pwn-lab.sh` 与各自 `toolchain.lock` 中锁定的 Ubuntu 24.04
  i686 交叉工具链可逐字节重建；目录为 `vm/labs/pwnhub/vuln-weak-random-01/`、
  `vuln-integer-overflow-01/`、`vuln-overwrite-variable-01/`、
  `vuln-string-overflow-01/`、`vuln-format-string-01/`、`vuln-race-condition-01/`。
- 判题方式按 `manifest.verification.type` 分三类：`answer`（进制、弱随机、整数溢出、
...
- 20 关严格串行链的 `unlockAfter` 与 `unlock-labs` 顺序为
  `num-bases-01 → num-wrap-01 → vuln-weak-random-01 → vuln-integer-overflow-01 →
  vuln-race-condition-01 → memory-addresses-01 → memory-layout-01 →
  memory-register-stack-01 → vuln-overwrite-variable-01 → vuln-string-overflow-01 →
  vuln-format-string-01 → asm-registers-01 → asm-arithmetic-01 → asm-stack-ops-01 →
  asm-branches-01 → asm-call-stack-01 → elf-bytes-01 → elf-sections-01 →
  elf-symbols-01 → elf-disassembly-01`，会被 `scripts/validate-binary-profile.mjs`
